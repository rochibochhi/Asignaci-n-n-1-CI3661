...
-- Nombre del estudiante: Rosa Ramirez
-- Carnet: 20-10527
-------------------------------------------------------------------------------
...
esPalindromo "" = True
esPalindromo [_] = True
esPalindromo s = (head s == last s) && esPalindromo (init (tail s))
...
parsearUna :: String -> Either String Int
parsearUna s =
  case reads s of
    [(n, "")] -> Right n
    _         -> Left (map toUpper s)

parsearCondicional :: [String] -> [Either String Int]
parsearCondicional = map parsearUna
